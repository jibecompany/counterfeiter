...
import (
	...
	"github.com/jibecompany/counterfeiter/v6/command"
...
func run() error {
	...
	for i := range invocations {
		...
		// '//go:generate go run github.com/jibecompany/counterfeiter/v6 -generate -header /some/header.txt'
...
func doGenerate(workingDir string, args *arguments.ParsedArguments, cache generator.Cacher, headerReader generator.FileReader) ([]byte, error) {
	...
	f, err := generator.NewFake(mode, args.InterfaceName, args.PackagePath, args.FakeImplName, args.DestinationPackageName, args.GenericType, args.IsGeneric, headerContent, workingDir, cache)
	if err != nil {
		return nil, err
	}
